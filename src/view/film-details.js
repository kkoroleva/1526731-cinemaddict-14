...
const renderFilmGenres = (genres) => {
  ...
  genres.forEach((genre)=>{
    genreBlock.push(`<span class="film-details__genre">${genre}</span>`);
  });
  return genreBlock.join('');
};

const renderComments = (comments) => {
  const commentBlock = [];
  comments.forEach(({emoji, text, author, date})=>{
    commentBlock.push(`<li class="film-details__comment">
        ...
        <div class="film-details__info">
          <div class="film-details__info-head">
            <div class="film-details__title-wrap">
              <h3 class="film-details__title">${names.name}</h3>
              <p class="film-details__title-original">Original: ${names.originalName}</p>
            </div>
...
export default class FilmDetails extends AbstractComponentView {
  constructor(film) {
    ...
    this._controlWatchlist = this.getElement().querySelector('#watchlist');
    this._controlWatched = this.getElement().querySelector('#watched');
    this._controlFavourite = this.getElement().querySelector('#favorite');
